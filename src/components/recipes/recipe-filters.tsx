  ...
  return (
    <div className="flex flex-col gap-3 mb-4 sm:mb-6 md:mb-8 sm:flex-row sm:gap-4">
      <form onSubmit={handleSearchSubmit} className="flex-1 flex gap-2 sm:gap-3">
        ...
        <Button type="submit" variant="secondary" disabled={isPending} className="h-10 sm:h-12 px-3 sm:px-6 text-sm sm:text-base">
      ...
      <div className="flex gap-2 sm:gap-4">
        ...
        >
          <SelectTrigger className="flex-1 sm:flex-none sm:w-[280px] !h-10 sm:!h-12 text-sm sm:!text-base bg-white dark:bg-stone-900 px-3 sm:px-4">
            <SelectValue placeholder="Catégorie" />
          </SelectTrigger>
          <SelectContent>
            {categories.map((cat) => (
              <SelectItem key={cat.value} value={cat.value} className="text-sm sm:text-base py-2 sm:py-3">
                {cat.label}
              </SelectItem>
            ))}
          </SelectContent>
        ...
        {hasFilters && (
          <Button variant="ghost" onClick={clearFilters} className="h-10 sm:h-12 gap-1 sm:gap-2 text-sm sm:text-base px-2 sm:px-4">
